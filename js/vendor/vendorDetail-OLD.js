...
async function displayVendor(vendor_id) {
    ...
    if (vendor != null){
        ...
        document.getElementById('vendor-title').textContent = vendor.name + " Company Info";
...
async function displayProducts(vendorId){
    ...
    if (hasProducts){
        ...
        for (const product of products)
           {
               const tr = document.createElement('tr');
               let price = await window.pywebview.api.vendor.get_product_price(product.id);
                let lastBill = product.updated_date;
                if (lastBill === undefined) {
                    lastBill = product.create_date;
                }
                if (!price){
                    price = {
                        price: '0.00'
...
async function addProduct() {
    ...
    if(productPrice.value === '')  {
        ...
        failure.classList.add('gs-alert', 'gs-alert--danger');
        failure.id = "error-message";
        failure.innerHTML = `<span class="gs-alert__lable">REQUIRED FIELD. </span> A product price is required.
            <span><button class="gs-btn gs-btn--sm gs-btn--danger" onclick="closeMessage()">close</button></span>`;
    ...
    let newProduct = {
        ...
        is_used: document.getElementById('product-isUsed').checked ? 1 : 0,
        vendor_id: vendorId
    }
    result = await window.pywebview.api.vendor.add_product(newProduct);

    if (result){
        let newPrice = {
            product_id: result.id,
            price: document.getElementById('product-price').value,
            is_active: document.getElementById('product-isUsed').checked ? 1 : 0
        };
        priceResult = await window.pywebview.api.vendor.add_product_price(newPrice);
        ...
        div.innerHTML = `<span class="gs-alert__lable">Success</span> ${newProduct.name} has been created.
            <span><button class="gs-btn gs-btn--sm" onclick="closeMessage()">close</button></span>`;
        ...
        failure.classList.add('gs-alert', 'gs-alert--danger');
        failure.id = "error-message";
        failure.innerHTML = `<span class="gs-alert__lable">Failure</span> ${newProduct.name} has not been created.
            <span><button class="gs-btn gs-btn--sm" onclick="closeMessage()">close</button></span>`;
        message.appendChild(failure);
...
function closeMessage() {
    const div = document.getElementById('product-message');
    div.innerHTML = ``;
}
function clearProductForm() {
    ...
    document.getElementById('product-isUsed').checked = false;
